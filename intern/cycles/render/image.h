...
public:
	...
	function<void(const string &filename, void *data, bool &is_float, int &width, int &height, int &depth, int &channels, int &num_ptex_rects)> builtin_image_info_cb;
	function<bool(const string &filename, void *data, unsigned char *pixels, PtexRegions ptex_rects, int num_ptex_rects)> builtin_image_pixels_cb;
...
private:
	...
	template<typename T>
	bool file_load_byte_image(Image *img, ImageDataType type, device_vector<T>& tex_img, int slot);
